...
def read_fasta_alignment(filename):
	...
	if line[0] == '>':
	    ...
	    if cur_seq != '':
		cur_seq = cur_seq.translate(None,string.ascii_lowercase)
	...
	elif line[0] not in iupac_alphabet: 
	    cur_seq += line.replace('\r', '') 
...
for i, seq in enumerate(alignment):
    seq_clean = seq.translate(None,string.ascii_lowercase)
    if len(seq_clean) != seq_len:
	print "ERROR: Sequences of different lengths: %s (%d) != %s (%d).\n" % (names[0], seq_len, names[i], len(seq_clean))
	print(seq_clean)
	sys.exit(1)
